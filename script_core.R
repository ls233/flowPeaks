...
library(flowPeaks)
source("script_heatmap.R")
if(.DEBUG){
    args<-c("fromJson_heatmap.txt")
}else{
...
processinput<-function(input){
    ...
    heatmap.plot<-function(n,K)
    {
        ...
        if(!is.wholenumber(n) | !is.wholenumber(K)){
            output$message<<-"The n or K were not integers"
            return(FALSE)
        }else{
            if(n<0 | n>10000 | K<0 |K>1000){
                output$message<<-"n or K is out of range"
                return(FALSE)
            }
        }
        datename<-gsub(" ","_",date(),fixed=TRUE)
        datename<-gsub(":","_",datename,fixed=TRUE)
        pngfile<<-paste("plot_heatmap_",datename,".png",sep="")
        script.plot(fp,n,K,file.path(odir,pngfile))
        return(TRUE)
    }
    
    scatter.plot<-function(ncol)
    {
        ix<-input$dims$x
        iy<-input$dims$y
    ...
    if(input$mode=="init"){
        ...
        if(input$mode=="refine"){
            ...
            adjust.flowPeaks(fp,tol=para$tol,h0=para$h0,
                             h=para$h)
            ##redo the heatmap plot
            if(!heatmap.plot()){
                return(output)
            }
        }else if(input$mode=="plot-scatter"){
            if(!scatter.plot(ncol(fp$x))){
                return(output)
            }
        }else if(input$mode=="plot-heatmap"){
            if(!heatmap.plot(para$n,para$K)){
                return(output)
            }
        }else{
    ...
    if(!input$mode %in% c("plot-scatter","plot-heatmap")){
        ...
        output$info<-list(n=500,K=max(fp$info[,1]),filedir=odir,pngfile=pngfile,fpfile="fpfile.fp",
                          txtfile=list(membership="membership.txt",
                          statistics="stat.txt"))
    }else{
        if(input$mode=="plot-scatter"){
